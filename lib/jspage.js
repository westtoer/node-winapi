...
module.exports.doSplit = function (size, dirOutput, tplPageName, fInput) {
    ...
    function realSplit() {
        ...
        function writeNext(data, isLast) {
            ...
            fs.writeFile(path.join(dirOutput, chunkname), JSON.stringify(data));

            if (isLast) {
                fs.writeFile(path.join(dirOutput, tplPageName + "index.json"), JSON.stringify(chunksList));
            }
        }
        ...
        try {
            ...
            console.log("error splitting json file " + fInput);
        }
    }
