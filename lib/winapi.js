...
GenericQuery.prototype.requirePubChannel = function () {
    return this.andField("publishing_channels.tdms__publishing_channel.@code");
};
...
GenericQuery.prototype.getURI = function (client, notoken) {
    ...
    GenericQuery.addQueryVal(q, "metadata.tdms__id", this.selectId);
...
Client.prototype.parseVocabularyCodes = function (vocabs, onlyNames) {
    return vocabs.reduce(function (byName, voc) {
        var name = voc.tdms__machine_name;
        if (onlyNames === undefined || onlyNames.indexOf(name) !== -1) {
            if (byName.hasOwnProperty()) {
                throw new Error("duplicate key tdms__machine_name : " + name);
            }
            if (voc && voc.tdms__terms && voc.tdms__terms.tdms__term) {
                byName[name] = voc.tdms__terms.tdms__term.map(function (term) {
                    var code = term['@code'] || "";
                    if (code.length === 0) {
...
Client.prototype.parseVocabularyTrees = function (vocabs, onlyNames) {
    return vocabs.reduce(function (byName, voc) {
        var name = voc.tdms__machine_name, termRoot = {children: []}, termNodes;
        if (onlyNames === undefined || onlyNames.indexOf(name) !== -1) {
