...
Client.prototype.parseVocabularyCodes = function (vocabs) {
    return vocabs.reduce(function (byName, voc) {
        if (byName.hasOwnProperty(voc.tdms__machine_name)) {
            throw new Error("duplicate key tdms__machine_name : " + voc.tdms__machine_name);
        }
        if (voc && voc.tdms__terms && voc.tdms__terms.tdms__term) {
            byName[voc.tdms__machine_name] = voc.tdms__terms.tdms__term.map(function (term) {
                var code = term['@code'] || "";
                if (code.length === 0) {
                    throw new Error("invalid empty code '' for vocabulary: " + voc.tdms__machine_name);
                }
                return code;
            });
        } else {
            byName[voc.tdms__machine_name] = [];
        }
        return byName;
    }, {});
};

Client.prototype.parseVocabularyTrees = function (vocabs) {
    return vocabs.reduce(function (byName, voc) {
        if (byName.hasOwnProperty(voc.tdms__machine_name)) {
            throw new Error("duplicate key tdms__machine_name : " + voc.tdms__machine_name);
        }
        var termRoot = {children: []}, termNodes;
        if (voc && voc.tdms__terms && voc.tdms__terms.tdms__term) {
            // list all linkable nodes
            termNodes = voc.tdms__terms.tdms__term.reduce(function (tns, term) {
                var code = term['@code'] || "", pcode = term['@parent_code'] || "";
                if (code.length === 0) {
                    throw new Error("invalid empty code '' for vocabulary: " + voc.tdms__machine_name);
                }
                if (tns.hasOwnProperty(code)) {
                    throw new Error("duplicate code '" + code + "' for vocabulary: " + voc.tdms__machine_name);
                }
                tns[code] = {
                    code: code,
                    parent: pcode,
            ...
            Object.keys(termNodes).forEach(function (code) {
                ...
                if (pn === null || pn === undefined) {
                    throw new Error("not found node for parent_code '" + tn.parent + "' in voc named: " + voc.tdms__machine_name);
                }
                tn.parent = pn;
                pn.children.push(tn);
            });
        }
        byName[voc.tdms__machine_name] = termRoot.children;
        return byName;
    }, {});
};
...
module.exports.query = function (service) {
    ...
    if (service === 'statistics') {
        return new GenericQuery().forStatsOfYear();
    }
