...
ProductQuery.prototype.getURI = function (client) {
    ...
    if (this.sizeVal !== Client.DEFAULT_SIZE) {
        uri += "&size=" + this.sizeVal;
    }
    if (this.pageNum !== Client.DEFAULT_PAGE) {
        uri += "&page=" + this.pageNum;
    }
...
function streamData(uri, sink, verbose) {
    getResponse(uri, function (e, res) {
        if (e) {
            throw e;
            //alternative? sink.emit('error', e);
        } //else
        if (res.statusCode !== 200) {
            throw "error reading uri [" + uri + "] to stream - response.status == " + res.statusCode;
        } // else
        res.pipe(sink);
    }, verbose);
}
